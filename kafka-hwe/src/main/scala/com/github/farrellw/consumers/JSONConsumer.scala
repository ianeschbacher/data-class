    ...
    while ( {
      ...
      records.forEach((record: ConsumerRecord[String, String]) => {
        ...
        */
        ...
        //val jsonMessage = json.extract[Message]
       // println("JSON Name:"+jsonMessage.name)
        val message = record.value()

        val jValue = parse(record.value)
        val jsonMessage = jValue.extract[Message]
        println("Parsed name here:" + jsonMessage.name+"|Parsed username here:"+jsonMessage.username+"|Parsed email here:"+jsonMessage.email)
